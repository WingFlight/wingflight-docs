...
receiver's signal is lost, the flight controller instantly takes over all
RC channels -- including aux/mode switches, so arm state and flight-mode
switching keep working -- from the SBUS-in port instead. The moment the
main link's signal comes back, control instantly reverts to it. No
failsafe-stage delay applies in either direction.
...
1. Wire an SBUS-capable satellite receiver to a spare UART, then assign
   that port the **SBUS-In Fallback** function from the port function
   dropdown on the Configuration tab's serial ports list.
2. Bind the satellite as you would any SBUS receiver. If it isn't decoding
   (Link stays down on the status panel below), check wiring against the
...
3. Confirm it's working from the SBUS-In Fallback status shown on the
   [FBUS/S.Port Sensors](../configurator/tabs/fbus-sensors.md) tab --
   live channel values and link-up state are shown there whenever a port
   is assigned this function, independent of whether FBUS/S.Port master
   is also in use.

## Behavior

- **Instant, not staged.** This does not go through the normal
  [Failsafe](../configurator/tabs/failsafe.md) stage machine at all -- it
  reacts to main-link signal presence directly, so failover and recovery
  each happen within about one main-RX frame period, with no
  `failsafe_delay`/`failsafe_recovery_delay` involved.
...
- **Auto-revert.** As soon as the main receiver's signal is valid again,
  control reverts to it immediately -- fallback does not latch for the
  rest of the flight.
...
1. With both the main receiver and the SBUS-in satellite bound and
   powered, confirm the FBUS/S.Port Sensors tab shows the main link
...
2. Power off (or walk the main receiver's transmitter out of range),
   and confirm control switches to the SBUS-in satellite almost
   instantly, arm state is preserved, and aux switches on the fallback
   radio work as expected.
3. Restore the main link and confirm control reverts to it immediately.
